...
func parser(log *xlog.Logger, output []byte) (map[string][]linters.LinterOutput, error) {
	var lineParser = func(line string) (*linters.LinterOutput, error) {
		...
		// If there are no detectable files in the current directory, cppcheck will display the error message "cppcheck: error: could not find or open any of the paths given."
		...
		return linters.GeneralLineParser(strings.Trim(line, "'"))
	}
